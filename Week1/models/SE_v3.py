...
class SE_v3(nn.Module):
    ...
    def __init__(self, input_shape=(256,256,3), num_classes=10):
        ...
        # First block
        self.conv_1_1 = nn.Conv2d(in_channels=self.input_shape[-1], out_channels=32, kernel_size=5, strides=2, padding=0)
    ...
    def forward(self, x):
        ...
        b, c, _, _ = x.size()
        y = self.avgpool_2_1(x).view(b,c,1,1)
        y = self.dense_2_1(y)
        y = self.relu_2_2(y)
        y = self.dense_2_2(y)
        ...
        b, c, _, _ = x.size()
        y = self.avgpool_3_1(x).view(b,c,1,1)
        y = self.dense_3_1(y)
        y = self.relu_3_2(y)
        y = self.dense_3_2(y)
        ...
        b, c, _, _ = x.size()
        y = self.avgpool_4_1(x).view(b,c,1,1)
        y = self.dense_4_1(y)
        y = self.relu_4_2(y)
        y = self.dense_4_2(y)
        ...
        b, c, _, _ = x.size()
        y = self.avgpool_5_1(x).view(b,c,1,1)
        y = self.dense_5_1(y)
        y = self.relu_5_2(y)
        y = self.dense_5_2(y)
